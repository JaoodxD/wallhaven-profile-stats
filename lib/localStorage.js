...
CREATE TABLE IF NOT EXISTS images_tags (
  ...
  FOREIGN KEY (tag_id) REFERENCES tags(wh_tag_id)
);
`

const CREATE_IMAGE_SQL = `
REPLACE INTO images (wh_image_id,   colors,  path,  url)
            VALUES (@wh_image_id, @colors, @path, @url)
RETURNING *;
`

const CREATE_TAG_SQL = `
REPLACE INTO tags (name, wh_tag_id) VALUES (@name, @wh_tag_id) RETURNING *;`
...
export default function LocalStorage (login) {
  ...
  function addTag ({ name, id }) {
    const payload = { name, wh_tag_id: id }
