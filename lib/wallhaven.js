...
export default function wallhaven (API_KEY, username) {
  const opts = { interval: 60 * 1000, intervalCap: 45 }
  const request = rateLimiter(fetch, opts)
  return {
    wpInfo,
    wpTags,
    tagInfo,
  ...
  async function collectionInfo (id) {
    const url = `${apiUrl}/collections/${username}/${id}`
    const response = await request(url)
    const { data } = await response.json()
    return data
  }
